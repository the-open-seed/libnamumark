...
void ast_node_print(ast_node *node){
    ...
    while (n_stack->size > 0) {
        ...
        if(c_node->data_type == AST_DATA_TYPE_STRING) {
            ...
            printf("├color: %s(%zu)\n", ((ast_data_color*)c_node->data)->color, ((ast_data_color*)c_node->data)->color_size);
            for(size_t i = 0; i <= depth; i++){
                printf("│  ");
            }
            printf("├dark_color: %s(%zu)\n", ((ast_data_color*)c_node->data)->dark_color, ((ast_data_color*)c_node->data)->dark_color_size);
        }
        for (size_t i = 0; i < depth; i++) {
            printf("│  ");
        }
